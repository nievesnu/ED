...
public:
	...
	/** Operation to merge the parmeter list into the current list,
	emptying the former one and getting the result in the current list*/
	void merge(List<T>& l2) {
		Node* n1, * n2, *prevN1=NULL, *auxNext2;
		// Update the number of elements
		this->_numElems += l2._numElems;
		n1 = _first;
		n2 = l2._first;
		// Update First pointer if necessary
		if (_first == NULL || ((l2._first!=NULL) && (l2._first->_elem <= _first->_elem))) {
			_first = l2._first;
		}
		...
		// Update lasta element if necessary
		if (n1 == NULL && prevN1!=NULL) {
			prevN1->_next = n2;
			n2->_prev = prevN1;
			_last = l2._last;
		}
...
private:
	...
	*/
	...
	// Pointer to the first and the last
	Node* _first, * _last;
